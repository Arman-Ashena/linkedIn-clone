...
const Widgets = ({ articles }) => {
  return (
    <div className="hidden md:inline space-y-2">
      <div
        className="bg-white dark:bg-[#1D2226] py-2.5 rounded-lg space-y-2 w-11/12 overflow-hidden
      border border-gray-300 dark:border-none"
      >
        <div className="flex items-center justify-between font-bold px-2.5">
          <h4>LinkedIn News</h4>
          <InfoRoundedIcon classNameh-5 w-5 />
        </div>
        <div className="space-y-1">
          {articles.slice(0, 5).map((article) => (
            ...
            >
              ...
              <div>
                <h5 className="max-w-xs font-medium text-xs truncate pr-10">
                  {article.title}
                </h5>
                <TimeAgo
      ...
      <div className="bg-white dark:bg-[#1D2226] w-11/12 h-64 px-2.5 rounded-lg sticky top-20 border border-gray-300 dark:border-none">
        <div className="relative w-full h-full">
          <Image src={job} layout="fill" objectFit="contain" property />
        </div>
      </div>
    </div>
  );
